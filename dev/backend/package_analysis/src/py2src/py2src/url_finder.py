...
class GetFinalURL:
    def __init__(self, package_name: str, ecosystem: str='pypi'):
...
class URLFinder:
    ...
    def find_ossgadget_url(self, type: str) -> str:
        ...
        command = ["docker", "run", "--rm", "pakaremon/ossgadget:latest",  "oss-find-source pkg:" + pkg]
        print(command)

        
        try:
            result = subprocess.run(" ".join(command), check=True, shell=True, stdout=subprocess.PIPE)
            # print stdout
            # print(result.stdout)
            # print(result.stderr)
    ...
    def launch_ossgadget_command(pkg: str) -> str:
        ...
        result = subprocess.run(" ".join(command), shell=True, check=True, stdout=subprocess.PIPE)
        decoded_result = result.stdout.decode('utf-8')
